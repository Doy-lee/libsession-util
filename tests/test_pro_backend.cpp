...
TEST_CASE("Session Pro Backend C API", "[session_pro_backend]") {
...

    {
        ...
        SECTION("session_pro_backend_add_pro_payment_or_get_pro_proof_response_parse") {
                            ...
                            json.data(), json.size());
            {
                ...
                REQUIRE(result.expiry_unix_ts_ms == unix_ts_ms);
                REQUIRE(std::memcmp(
                                result.gen_index_hash.data,
                                fake_gen_index_hash,
                                sizeof(fake_gen_index_hash)) == 0);
                REQUIRE(std::memcmp(
                                result.rotating_pkey.data,
                                rotating_pubkey.data,
                                sizeof(rotating_pubkey)) == 0);
                REQUIRE(std::memcmp(result.sig.data, master_privkey.data, sizeof(master_privkey)) ==
                        0);
                ...
                proof_cpp.from_pro_backend_response(result_cpp);
                ...
                REQUIRE(proof_c.expiry_unix_ts_ms ==
                        proof_cpp.expiry_unix_ts.time_since_epoch().count());
                REQUIRE(std::memcmp(proof_c.sig.data, proof_cpp.sig.data(), proof_cpp.sig.size()) ==
                        0);
            }
    ...
    SECTION("Send to local dev server") {
        ...
        pro_proof first_pro_proof = {};
        {
            ...
            // Verify response
            first_pro_proof = pro_proof_from_pro_backend_response(&response);
        ...
        // Authorise new key
        {
            ...
            // Verify response
            pro_proof proof = pro_proof_from_pro_backend_response(&response);
            REQUIRE(pro_proof_verify_signature(
